 ...
 *
 * FORKED VERSION with sendAudio() support for mid-call audio injection.
 *
 ...
 *   call.sendAudio(float32Array)  // NEW: Send audio mid-call
 *
 * @author ShellTear (original)
 * @modified Added sendAudio() support
 */
...
export class ActiveCall extends EventEmitter {
    ...
    // NEW: Audio queue for sendAudio() support
    _audioQueue = [];
    _chunkSize = 320; // samples per chunk (20ms at 16kHz)
    ...
    /**
     * NEW: Send audio during the call.
     * @param {Float32Array} pcmData - 16kHz mono float32 PCM audio (-1.0 to 1.0)
     *
     * The audio is queued and will be sent in 20ms chunks (320 samples).
     * This replaces the silence/file audio while the queue has data.
     */
    sendAudio = (pcmData) => {
        if (!pcmData || pcmData.length === 0) return;

        // Split into chunks of _chunkSize samples
        for (let i = 0; i < pcmData.length; i += this._chunkSize) {
            const end = Math.min(i + this._chunkSize, pcmData.length);
            const chunk = new Float32Array(this._chunkSize);

            // Copy data, padding with zeros if needed
            for (let j = 0; j < this._chunkSize; j++) {
                chunk[j] = (i + j < pcmData.length) ? pcmData[i + j] : 0;
            }
    ...
    /**
     * NEW: Check if there's queued audio.
     * @internal
     */
    _hasQueuedAudio = () => {
    ...
    /**
     * NEW: Get next audio chunk from queue.
     * @internal
     * @returns {Float32Array|null}
     */
    _getNextChunk = () => {
        if (this._audioQueue.length === 0) return null;
        return this._audioQueue.shift();
    };

    /**
     * NEW: Get queue length (for debugging).
     */
    getQueueLength = () => {
...
export class VoipClient {
    ...
    call = async (phoneNumber, opts = {}) => {
        ...
        call._chunkSize = this.#captureFramesPerChunk; // Sync chunk size
    ...
    #handleAudioCaptureInit = (config) => {
        ...
        // Sync chunk size with active call
        if (this.#activeCall) {
            this.#activeCall._chunkSize = this.#captureFramesPerChunk;
        }
    };
    #handleAudioCaptureStart = () => {
        ...
        // MODIFIED: Check for queued audio from sendAudio() before using feeder audio
        this.#feeder = new AudioFeeder(this.#captureSampleRate, this.#captureChannels, this.#captureFramesPerChunk, (chunk) => {
            ...
            // Check if the active call has queued audio
            if (this.#activeCall && this.#activeCall._hasQueuedAudio()) {
                const queuedChunk = this.#activeCall._getNextChunk();
                if (queuedChunk) {
                    this.#engine.sendAudioData(queuedChunk, this.#capturePtr);
                    return;
                }
            }

            // Otherwise use the feeder audio (from file or silence)
