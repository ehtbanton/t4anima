...
class PgxAgent:
    ...
    def scan(self, patient_ids: Iterable[str] | None = None,
        ...
        if patient_ids:
            for pid in patient_ids:
                for site in sites:
                    try:
                        page = self.sim.view(site, patient=pid, limit=200)
    ...
    def render(self, prop: Proposal) -> str:
        ...
        if prop.blocked_by:
            ...
            lines += [
                ...
                f"    1. issue the replacement regimen above",
    ...
    def authorise(self, task_id: str, clinician: str, approve: bool = True,
        ...
        if raw.get("status") in ("authorised", "rejected", "applied"):
        ...
        if not regimen:
            raise ValueError("proposal has no drafted regimen to apply")
        ...
        if clash:
            ...
            self.ledger.event("apply_skipped_duplicate", task=task_id, patient=pid,
                              existing=clash)
            self.ledger.save()
            return {"status": "superseded", "clinician": clinician,
                    "note": "replacement regimen is already on the record",
                    "existing": clash}
        ...
        try:
            ...
            self.sim.message_patient(
                ...
                body=(
                    ...
                    "record and changed one of your medicines so it works properly "
                    "for you. Your new prescription is ready. Please stop taking "
                    f"{raw['current_drug']}. Contact the practice if you have questions. "
        ...
        self.ledger.event("applied", task=task_id, clinician=clinician,
                          patient=pid, issued=issued, cancelled=cancelled,
                          errors=errors)
        self._close_task(task_id, raw, clinician)
        self.ledger.save()
        return {"status": "applied", "clinician": clinician, "issued": issued,
